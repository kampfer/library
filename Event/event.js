	...
	kampfer.event = {
		...
		addEvent : function ( elem, type, fn ) {
			...
			if( elem.nodeType === 3 || elem.nodeType === 8 || !type ||
				!fn || !(elemData = kampfer._data(elem)) ) {
				return;
			}
		...
		removeEvent : function( elem, type, fn ) {
			...
			var elemData = kampfer.hasData( elem ) && kampfer._data( elem ),
				events = elemData.events, 
			...
			if( kampfer.isEmptyObject( events ) ) {
				bindHandler = elemData.handler;
				if( bindHandler ) {
					bindHandler.elem = null;
				}
				kampfer.removeData( elem, ['evnets', 'handler'], true );
			}
		...
		trigger : function( elem, event ) {
			...
			for( i = 0, l = eventPath.length; i < l; i++ ) {
				...
				handler = (kampfer._data( cur, 'events' ) || {})[type] && 
					kampfer._data( cur, 'handler' );
				if( handler ) {
		...
		dispatch : function( event ) {
			...
			var handlers = (kampfer._data( this, 'events' ) || {})[event.type] || [],
				i, l, fn, ret;
